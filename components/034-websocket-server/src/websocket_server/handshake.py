...
def parse_http_headers(raw_request: bytes) -> Tuple[str, Dict[str, str]]:
    """
    Parses raw HTTP request bytes and returns the request line and a dictionary of headers.

    Args:
        raw_request: The raw bytes of the HTTP request.

    Returns:
        A tuple of (request_line, headers_dict).
    """
    if not raw_request:
...
def validate_handshake(request_line: str, headers: Dict[str, str]) -> str:
    """
    Validates the handshake and returns the Sec-WebSocket-Key.
    ...
    Raises:
        HandshakeError: If the handshake is invalid.
    """
    if not request_line.startswith("GET"):
        raise HandshakeError("Method must be GET")
